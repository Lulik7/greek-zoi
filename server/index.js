...
/*
 * Основной адрес сайта — тот, что задан в PUBLIC_URL. Всё остальное (служебный
 * адрес хостинга вида …onrender.com, адрес с www) переадресуем на него: иначе
 * получается вторая копия сайта. Поисковик может проиндексировать её отдельно,
 * а ученик, начавший оплату на копии, вернётся со Stripe уже на основной адрес
 * и без входа — вход привязан к тому адресу, где заводили аккаунт.
 *
 * В разработке адрес сайта и адрес API разные (5173 и 4000), там переадресация
 * только мешала бы, поэтому локальные адреса пропускаем.
 */
const canonicalHost = (() => {
  try {
    const { host, hostname } = new URL(config.publicUrl);
    return hostname === 'localhost' || hostname === '127.0.0.1' ? '' : host;
  } catch {
    return ''; // адрес в настройках кривой — лучше вообще никуда не перекидывать
  }
})();
...
app.use((req, res, next) => {
  ...
  /*
   * Запросы к /api/ не трогаем: по адресу /api/bootstrap хостинг проверяет,
   * жив ли сервис, и ответ-переадресацию счёл бы поломкой. Ученику это ничем
   * не грозит — страницу сайта он получит уже с основного адреса, и все
   * запросы к API уйдут оттуда же.
   */
  if (
    canonicalHost &&
    req.headers.host &&
...
app.post('/api/admin/mail-test', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const hints = {
      NOSMTP:
        'Почта не настроена: не заполнен ни ключ почтового сервиса (RESEND_API_KEY или ' +
        'BREVO_API_KEY), ни SMTP_HOST.',
      EAUTH:
        'Почтовый сервис не принял ключ. Проверьте, что ключ скопирован целиком и без ' +
        'пробелов. Для Gmail через SMTP нужен «пароль приложения» (16 букв), а не обычный ' +
        'пароль от почты.',
      EFROM:
        'Почтовый сервис не принял адрес отправителя. Проверьте переменную MAIL_FROM: ' +
        'адрес в угловых скобках должен быть на подтверждённом домене — в Resend это ' +
        'ваш домен (например, no-reply@ваш-домен), в Brevo — подтверждённый адрес.',
      EPROVIDER: 'Почтовый сервис ответил ошибкой. Подробности ниже.',
      ETIMEDOUT:
    ...
    const hint = hints[e.code] ?? 'Письмо не отправилось.';
    console.warn('[mail] пробное письмо не ушло:', e.code, e.message);
    res.status(400).json({
      // ответ сервиса дописываем к объяснению: по нему видно, что именно не так
      error: `${hint}\n\nОтвет сервиса: ${e.message}`,
      code: e.code ?? '',
      ms: Date.now() - started,
    });
  }
